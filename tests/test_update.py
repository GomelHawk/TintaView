...
def test_only_the_latest_release_endpoint_is_ever_asked(monkeypatch):
    """`/releases/latest` already excludes drafts and pre-releases, which is the whole
    reason it is the only endpoint read. A second `update.channel = "beta"` once read
    the release *list* to pick pre-releases up; it was removed (nothing here has ever
    been tagged as one) and must not come back unasked — see AGENTS.md."""
